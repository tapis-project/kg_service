...
class Networking(TapisModel):
    ...
    tapis_auth_response_headers: list[str] = Field([], description = "List of headers to forward to the pod when using Tapis auth.")
    ...
    def check_tapis_auth_forward_cookies(cls, v):
        if v:
            if not isinstance(v, list):
                raise TypeError(f"tapis_auth_response_headers must be list. Got '{type(v).__name__}'.")
            for header in v:
                if not isinstance(header, str):
                    raise TypeError(f"tapis_auth_response_headers must be list of str. Got '{type(header).__name__}'.")
        return v
